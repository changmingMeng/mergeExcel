...
def submit():    
	...
	root = Tk()
	root.title("测试")
	frame = Frame(root)frame.pack(padx=8, pady=8, ipadx=4)
	...
	x = (root.winfo_screenwidth() - root.winfo_reqwidth()) / 2y = (root.winfo_screenheight() - root.winfo_reqheight()) /2
	root.geometry("+%d+%d" % (x, y))root.mainloop()